library(nnet)
library(tidyverse)
#projectpath='/shared/mfs/data/home/croux/scratch/morpho/bin/dev'
#binpath='/shared/mfs/data/home/croux/scratch/morpho/bin'
#timeStamp='Uhfd8fd'
#nIterations=200
#ncores=8
...
for(tmp in commandArgs()){
	...
	if(tmp[[1]][1] == 'analysis'){ analysis = tmp[[1]][2] }
}
...
if(analysis == 'optimization1'){
	subPath = 'optimization1'
}

setwd(subPath)

for(i in 0:(nIterations-1)){
	file_prior = paste('bestModel_', i, '/priorfile.txt', sep='')
	file_stats = paste('bestModel_', i, '/ABCstat.txt', sep='')

	tmp = as.matrix(fread(file_prior, h=T))
	prior = rbind(prior, tmp)
	
	tmp = read.table(file_stats, h=T)[, -toReject]
	simSumStats= rbind(simSumStats, tmp)
}


transf = rep('logit', ncol(prior))
bb = rbind(as.numeric(apply(prior, MARGIN=2, FUN='min')), as.numeric(apply(prior, MARGIN=2, FUN='max')))

tmp = apply(prior, MARGIN=2, FUN="var")
if(0%in%tmp){
	noVar = which(tmp == 0)
	transf[noVar] = 'none'
	bb[,noVar] = c(0, 0)
...
res = abc_nnet_multivar(target=obs[-toReject], x=prior, sumstat=simSumStats, tol=1000/nrow(prior), transf=transf, bb=bb)

posterior = res$x
colnames(posterior) = colnames(prior)
posterior = as_tibble(posterior)
posterior2 = replace(posterior, posterior<0, 0.00001)
#posterior2 =  posterior2 %>% mutate_if(is.numeric, round, 5)
write.table(x=posterior2, file=paste(subPath, '.txt', sep=''), sep='\t', quote=F, row.names=F, col.names=T)
