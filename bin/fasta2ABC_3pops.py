...
help = 'fasta2ABC_3pops.py Morpho_Peru_dils.fa morpho_3sp hel_P ach_P dei_P NA noncoding 100 0.1 6 7 8 100000 0.0000001 1 /shared/home/croux/scratch/morpho/bin'
# check the arguments
if len(sys.argv) != 17:
	print("\n\tfasta2ABC_3pops.py produces: bpfile (for simulations) and summary statistics (for inferences)")
	print("\n\033[1;33m\tExample: ./fasta2ABC_3pops.py all_loci.fasta flo mal coding 30 0.1 10 100000 0.00000002 1\033[0m\n")
	...
	print("\t\targ5 =\tID of species C (example: ros)")
	print("\t\targ6 =\tID of the outgroup (exmaple: num)")
	print("\t\targ7 =\t'coding' or 'noncoding', to study only synonymous polymorphisms (if coding) or all SNPs (if noncoding)")
	print("\t\targ8 =\tminimum length of a locus to be considered, i.e, number of total positions minus the positions containing a N")
	print("\t\targ9 =\tvalue in [0-1]. Corresponds to a threshold of %N above which a sequence is rejected")
	print("\t\targ10 =\tinteger, corresponding to the minimum number of retained sequences (after rejection).\n\t\t\tif not enough sequences are retained, the loci is excluded from the analysis")
	print("\t\targ11 =\tsize of the reference population, arbitrary fixed. i.e: Nref=100000")
	print("\t\targ12 =\tmutation rate by bp and by generation. example: 0.00000002")
	print("\t\targ13 =\tratio of the recombination rate over mutation. example: 1")
	print("\t\targ14 =\tpathway to the binary files of DILS: /shared/mfs/data/home/croux/softwares/ABConline/bin")
	if(len(sys.argv)<17):
		print(help)
		sys.exit("\n\033[1;31m ERROR in fasta2ABC_3pops.py: 17 arguments are required: {0} missing\033[0m\n".format(17-len(sys.argv)))
	if(len(sys.argv)>17):
		print(help)
		sys.exit("\n\033[1;31m ERROR in fasta2ABC_3pops.py: 17 arguments are required: {0} too much\033[0m\n".format(len(sys.argv)-17))
...
nameC = sys.argv[5] # name of species C. example: ros 
nameOut = sys.argv[6] # name of the outgroup. example: num
region = sys.argv[7] # if == coding: will only deal with synonymous codons; if == noncoding: will deal with all positions
Lmin = int(sys.argv[8]) # minimum length for a locus to be retained. example: 30
max_N_tolerated = float(sys.argv[9]) # if an allele has %N > threshold_N --> sequence is rejected
nMinA = int(sys.argv[10]) # minimum number of individuals within a species. example: 10
nMinB = int(sys.argv[11]) # minimum number of individuals within a species. example: 10
nMinC = int(sys.argv[12]) # minimum number of individuals within a species. example: 10
Nref = int(float(sys.argv[13])) # size of the reference population, arbitrary fixed. i.e: Nref=100000
mu = float(sys.argv[14]) # mutation rate by bp and by generation. example: 0.00000002
rho_over_theta = float(sys.argv[15]) # ratio of the recombination rate over mutation. example: 1
binpath = sys.argv[16] # pathway to the directory of DILS (fastABC / ABConline) containing all executable files
...
if nameA == nameB:
	sys.exit("\n\t\033[1;31m ERROR in fasta2ABC_2pops.py: argument #2 ({0} provided here) and argument #3 ({1}) have to be different\033[0m\n".format(nameA, nameB))
	
if nameA == nameC:
	sys.exit("\n\t\033[1;31m ERROR in fasta2ABC_2pops.py: argument #2 ({0} provided here) and argument #4 ({1}) have to be different\033[0m\n".format(nameA, nameC))
	
if nameB == nameC:
	sys.exit("\n\t\033[1;31m ERROR in fasta2ABC_2pops.py: argument #3 ({0} provided here) and argument #4 ({1}) have to be different\033[0m\n".format(nameB, nameC))

if region not in ['coding', 'noncoding']:
	sys.exit("\n\t\033[1;31m ERROR in fasta2ABC_3pops.py: '{0}' is not an expected argument. 'coding' or 'noncoding' are expected here\033[0m\n".format(region))
...
def fasta2list(fastaFile, nameA, nameB, nameC, nameOut, nMinA, nMinB, nMinC, max_N_tolerated):
	L = {}
	...
	if nameOut == 'NA':	
		list_species = [nameA, nameB, nameC]
	else:
		list_species = [nameA, nameB, nameC, nameOut]
	...
	for i in nsam: # loop along loci
		if nameOut == 'NA':
			if nsam[i][nameA] < nMinA or nsam[i][nameB] < nMinB or nsam[i][nameC] < nMinC:
				...
				sub_sample_A = random.sample(range(nsam[i][nameA]), nMinA)
				...
				sub_sample_B = random.sample(range(nsam[i][nameB]), nMinB)
				...
				nsam[i][nameB] = nMinB
				# sub sample nMin sequences from species C
				sub_sample_C = random.sample(range(nsam[i][nameC]), nMinC)
				sub_seq_C = [ res[nameC][i]['seq'][seq] for seq in sub_sample_C ]
				sub_id_C = [ res[nameC][i]['id'][seq] for seq in sub_sample_C ]
				res[nameC][i]['seq'] = sub_seq_C
				res[nameC][i]['id'] = sub_id_C
				nsam[i][nameC] = nMinC
		else:
			if nsam[i][nameA] < nMinA or nsam[i][nameB] < nMinB or nsam[i][nameC] < nMinC or nsam[i][nameOut]==0:
				for j in res.keys(): # loop over species
					if i in res[j]:
						del res[j][i] # delete locus i
			else:
...
def getScalar(align, L, consensus, region, nameA, nameB, nameC):
	...
	for locus in L: # loop over loci
		if locus in align[nameA] and locus in align[nameB] and locus in align[nameC]:
			nLocus += 1
			...
			for pos in list_positions: # loop over third coding positions
				...
				for seqB in align[nameB][locus]['seq']: # loop over indB
					B = seqB[pos]
					if B!='N' and out!='N':
						nComb += 1
						if B!=out:
							divergence_locus += 1.0
				for seqC in align[nameC][locus]['seq']: # loop over indC
					C = seqC[pos]
					if C!='N' and out!='N':
						nComb += 1
						if C!=out:
...
# read the input file
align = fasta2list(fileName, nameA, nameB, nameC, nameOut, nMinA, nMinB, nMinC, max_N_tolerated)  # align[species][locus]['id', 'seq']


if len(align['align'][nameA]) == 0 or len(align['align'][nameB]) == 0 or len(align['align'][nameC]) == 0:
	sys.exit('\n\tERROR in fasta2ABC_2pops.py: no locus found in file {0} corresponding to a correct alignement between {1}, {2} and {3}\n'.format(fileName, nameA, nameB, nameC))
	if nameOut != 'NA':
		if len(align['align'][nameOut]) == 0:
			sys.exit('\n\tERROR in fasta2ABC_2pops.py: no locus found in file {0} corresponding {1}\n'.format(fileName, nameOut))


# if there is an outgroup -> make a concensus
if nameOut != 'NA':
	consensus = getConsensus(align['align'][nameOut], align['L']) # consensus[locus] = sequence
	scalar = getScalar(align['align'], align['L'], consensus, region, nameA, nameB, nameC) # with outgroup: locus specific mutation rate mu_i = mean(mu) * div_i / mean(div)
else:
	scalar = {}
...
bpfile_L1 = '# spA={0} spB={1} spC={2} Nref={3} mu={4}'.format(nameA, nameB, nameC, Nref, mu)
bpfile_L2 = [] # L
bpfile_L3 = [] # nA
bpfile_L4 = [] # nB
bpfile_L5 = [] # nC
...
#output_ms = "./msnsam tbs 20 -t tbs -r tbs tbs -I 3 tbs tbs tbs 0 etc...\n3579 27011 59243\n\n"
output_ms = "java -jar msms.jar tbs 20 -s tbs -t tbs -r tbs tbs -I 3 tbs tbs tbs 0 etc...\n3579 27011 59243\n\n"
outfile_ms = open('{0}/{1}_{2}_{3}.ms'.format(timeStamp, nameA, nameB, nameC), 'w')
outfile_ms.write(output_ms)

if nameOut == 'NA':
	# For coding loci
#	if region == 'coding':
#		output_info = "locusName\tL_including_N\tLsyno\tnSynSegSite\tnsamA\tnsamB\tnsamC\n"
#		outfile_info = open('{0}/{1}_{2}_{3}_infos.txt'.format(timeStamp, nameA, nameB, nsamC), 'w')
...
#			nC = len(align['align'][nameC][locus_i]['id'])
#			
#			L = align['L'][locus_i] 
...
#			for i in range(nC):
#				interspe.append(align['align'][nameC][locus_i]['seq'][i])
#				interspeName.append(align['align'][nameC][locus_i]['id'][i])
#
...
#				bpfile_L4.append(nB)
#				bpfile_L5.append(nC)
#				bpfile_L6.append(4*Nref*mu*nSites) # theta = 4.Nref.µ.L, where Nref is the diploid population size
#				bpfile_L7.append(4*Nref*mu*nSites*rho_over_theta)
#					
#				# informations about locus
#				res = "{0}\t{1}\t{2}\t{3}\t{4}\t{5}\t{6}\n".format(geneName, L, nSites, nSynSegSite, nA, nB, nC)
...
#		bpfile += '\t'.join([ str(i) for i in bpfile_L7 ]) + '\n'
#
...
#		
	# For non coding loci
	if region == 'noncoding':
		output_info = "locusName\tL_including_N\tL\tnSegSite\tnsamA\tnsamB\tnsamC\n"
		outfile_info = open('{0}/{1}_{2}_{3}_infos.txt'.format(timeStamp, nameA, nameB, nameC), 'w')
		outfile_info.write(output_info)
		for locus_i in align['L'].keys():
			geneName = locus_i
...
#				print('{0}: {1}'.format(locus_i, pos))
			...
			if nSites >= Lmin: # if the locus is big enough to be considered
				...
				bpfile_L4.append(nB)
				bpfile_L5.append(nC)
#				bpfile_L6.append(4*Nref*mu*nSites) # theta = 4.Nref.µ.L, where Nref is the diploid population size CRoux
				bpfile_L6.append(nSegSite) # nSNPs
				bpfile_L7.append(4*Nref*mu*nSites*rho_over_theta)
					
				# informations about locus
				res = "{0}\t{1}\t{2}\t{3}\t{4}\t{5}\t{6}\n".format(geneName, L, nSites, nSegSite, nA, nB, nC)
				outfile_info.write(res)
	...
	# For coding loci
#	if region == 'coding':
#		output_info = "locusName\tL_including_N\tLsyno\tnSynSegSite\tnsamA\tnsamB\tnameC\tmutation_scalar\n"
#		outfile_info = open('{0}/{1}_{2}_{3}_infos.txt'.format(timeStamp, nameA, nameB, nameC), 'w')
#		outfile_info.write(output_info)
#		for locus_i in align['L'].keys():
...
#					bpfile_L4.append(nB)
#					bpfile_L5.append(nC)
#					bpfile_L6.append(4*Nref*mu*nSites*scalar[locus_i]) # theta = 4.Nref.µ.L, where Nref is the diploid population size
#					bpfile_L7.append(4*Nref*mu*nSites*rho_over_theta*scalar[locus_i])
#						
#					# informations about locus
#					res = "{0}\t{1}\t{2}\t{3}\t{4}\t{5}\t{6}\t{7}\n".format(geneName, L, nSites, nSynSegSite, nA, nB, nC, scalar[locus_i])
#					outfile_info.write(res)
#					
#				#	res = ""
#				#	for i in range(len(interspe)):
...
#	
	# For non coding loci
	if region == 'noncoding':
		output_info = "locusName\tL_including_N\tL\tnSegSite\tnsamA\tnsamB\tnsamC\tmutation_scalar\n"
		outfile_info = open('{0}/{1}_{2}_{3}_infos.txt'.format(timeStamp, nameA, nameB, nameC), 'w')
		outfile_info.write(output_info)
		for locus_i in align['L'].keys():
			if locus_i in consensus:
				...
				if nSites >= Lmin: # if the locus is big enough to be considered
					...
					# informations about locus
					res = "{0}\t{1}\t{2}\t{3}\t{4}\t{5}\t{6}\t{7}\n".format(geneName, L, nSites, nSegSite, nA, nB, nC, scalar[locus_i])
					outfile_info.write(res)
...
if nameOut == 'NA':
	...
	outgroup_present = 1
commande = 'cat {0}/{1}_{2}_{3}.ms | pypy {5}/mscalc_3pop_observedDataset_SFS.py {0} {4}'.format(timeStamp, nameA, nameB, nameC, outgroup_present, binpath)
print(commande)
#os.system(commande)

# remove the useless ms file
commande = 'rm {0}/{1}_{2}_{3}.ms'.format(timeStamp, nameA, nameB, nameC)
#print(commande)
#os.system(commande)
